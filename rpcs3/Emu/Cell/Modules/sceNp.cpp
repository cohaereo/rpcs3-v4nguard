...
error_code sceNpBasicSetPresenceDetails2(vm::cptr<SceNpBasicPresenceDetails2> pres, u32 options)
{
	...
	sceNp.notice("sceNpBasicSetPresenceDetails2: state: %d", pres->state);
	sceNp.notice("sceNpBasicSetPresenceDetails2: title: %s", reinterpret_cast<const char*>(&pres->title[0]));
	sceNp.notice("sceNpBasicSetPresenceDetails2: status: %s", reinterpret_cast<const char*>(&pres->status[0]));
	sceNp.notice("sceNpBasicSetPresenceDetails2: comment: %s", reinterpret_cast<const char*>(&pres->comment[0]));
	sceNp.notice("sceNpBasicSetPresenceDetails2: data size: %d", pres->size);
	sceNp.notice("sceNpBasicSetPresenceDetails2: data offset: 0x%x", &pres->data);

	char hexData[SCE_NP_BASIC_MAX_PRESENCE_SIZE * 2 + 1];
	memset(hexData, 0, SCE_NP_BASIC_MAX_PRESENCE_SIZE * 2 + 1);
	for (int i = 0; i < pres->size; i++)
	{
		hexData[i * 2 + 0] = "0123456789abcdef"[pres->data[i] & 0x0f];
		hexData[i * 2 + 1] = "0123456789abcdef"[(pres->data[i] >> 4) & 0x0f];
	}

	sceNp.notice("sceNpBasicSetPresenceDetails2: data: %s", static_cast<char*>(hexData));
...
error_code sceNpBasicGetFriendPresenceByIndex(u32 index, vm::ptr<SceNpUserInfo> user, vm::ptr<SceNpBasicPresenceDetails> pres, u32 options)
{
	...
	// if (index == 0)
	// {
	// 	np::string_to_npid("grammus_mcgee", &user->userId);
	// 	np::string_to_online_name("grammus_mcgee", &user->name);
	// 	np::string_to_avatar_url("https://rpcs3.net/cdn/netplay/DefaultAvatar.png", &user->icon);

	// 	strcpy_trunc(pres->status, "In Orbit");
	// }
	...
	memcpy(&user->userId, &res_npid.value(), sizeof(SceNpId));
	strcpy_trunc(user->name.data, user->userId.handle.data);
	np::string_to_avatar_url("https://rpcs3.net/cdn/netplay/DefaultAvatar.png", &user->icon);

	// u8 testData[60] = {
	// 	// Offset 0x00000000 to 0x0000003B
	// 	0x4D, 0x49, 0xA7, 0xBD, 0xBD, 0x72, 0xED, 0x32, 0x0C, 0x8A, 0x20, 0x27,
	// 	0x20, 0xC0, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0x00, 0x00, 0xFF, 0x00, 0xFF,
	// 	0x00, 0x00, 0xD4, 0xDA, 0x27, 0xA8, 0x20, 0xC0, 0x4E, 0xE5, 0x73, 0x9A,
	// 	0xE5, 0x9D, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC1, 0x75, 0xE6, 0xCE,
	// 	0x30, 0x3B, 0x44, 0xBF, 0xAC, 0xA6, 0xD2, 0xD4, 0xD3, 0xC5, 0x8C, 0x3B};
	strcpy_trunc(pres->status, "In Orbit");
	// memcpy(pres->data, testData, 60);
	// pres->size  = 60;
	pres->state = SCE_NP_BASIC_PRESENCE_STATE_IN_CONTEXT;
